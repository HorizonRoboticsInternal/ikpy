...
def inverse_kinematic_optimization(chain, target_frame, starting_nodes_angles,
    ...
    if regularization_parameter is not None:
        def optimize_total(x):
            dist = np.abs(x - chain.active_from_full(starting_nodes_angles))
            regularization = np.sum(dist * regularization_parameter)
            return optimize_function(x) + regularization
    else:
    ...
    if optimizer == "scalar":
        ...
        res = scipy.optimize.minimize(optimize_scalar, starting_pos, bounds=real_bounds)
    elif optimizer == "least_squares":
        # We need to unzip the bounds
        real_bounds = np.moveaxis(real_bounds, -1, 0)
        res = scipy.optimize.least_squares(optimize_total, starting_pos,
                                           bounds=real_bounds)
